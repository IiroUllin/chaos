...
//
//  Vectorized versions for SIMD_BLOCK streams at once;
//  state[0] contains consecutive L-qwords; state[1] -- H-qwords
//
#ifdef __SIMD__

static inline vint64_t __rotl_vec(const vint64_t state, const int amount) {
    return _or(_shift_left(state, amount), _shift_right(state, 64 - amount));
}

static inline vint64_t __next_vec(vint64_t* state) {
    vint64_t s0 = state[0],
			 s1 = _xor(state[1], s0);

	// s0 = __rotl(s0, 24) ^ s1 ^ (s1 << 16);
    s0 = _xor(_xor(__rotl_vec(s0, 24), s1), _shift_left(s1, 16));
	s1 = __rotl_vec(s1, 37);

	state[0] = s0; state[1] = s1;

    return _add_int(s0, s1);
}
#endif
...
void chs::RNG::next(){
	...
	if (__builtin_expect(activeStream == STREAM_NUM, false)){   //  Expect that activeStream < STREAM_NUM
		...
		//
#ifdef __SIMD__                                 //------- Vectorization enabled ---------
		vint64_t *data = (vint64_t*) i64;                   //  C-style cast; i64 MUST be properly aligned, or this may crash!
		vfp64_t *floats = (vfp64_t*) f64;                   //  Same here.
		for (int i = 0; i < BLOCK_NUM; i++){
			*(floats++) = fp01_vec(__next_vec(data));       //  Process four 64 byte chunks at a time
			data += 2;                                      //  Increment by 2 (data++ would point to the H-qwords of the same states)
		}
#else                                           //------- Vectorization disabled ----------
		for (int i = 0; i < STREAM_NUM; i++) f64[i] = fp01(__next(&i64[_L[i]]));
#endif
		//
...
fp64_t chs::RNG::Exp1(){
	...
	fp64_t Y = (X - 1.0) / (X + 1.0),
		   Z = Y * Y,
		   //
		   //   This approximation min/max-es the error using the Remez algorithm, as Google claims.
